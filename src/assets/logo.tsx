...
const Logo: React.FC<LogoProps> = ({ 
  ...
  return (
    <div className={cn('font-bold flex items-center gap-2', sizeClasses[size], className)}>
      ...
      {variant === 'colored' && (
        <div className="relative w-6 h-6 animate-pulse-soft">
          <div className="absolute inset-0 bg-ninja-orange rounded-full opacity-70"></div>
          <div className="absolute inset-1 bg-ninja-green rounded-full"></div>
        </div>
      )}
    </div>
  );
